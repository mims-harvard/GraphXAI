...
def calculate_edge_exp_metrics(node_idx, subset, sub_edge_index, mapping, sub_x, metrics, model, device,
    ...
    pgm_explanation = explainer.pgm_generate(ind, data, stats, subnodes)
    salient_node = [int(aa) for aa in pgm_explanation.nodes()]
    counter_node_explanation = torch.zeros(sub_x.shape[0])
    counter_node_explanation[salient_node] = 1
    ...
    if preds_x[mapping].item() == preds_counter_x[mapping].item():
        counter_fair_score = metrics.dist_explanation(node_explanation, counter_node_explanation).item()
    else:
    ...
    return lg_pred_score, stab_score, counter_fair_score, lg_fair_score, x_norm, lg_softmax_score, \
           lg_norm_sm_faithfulness, torch.cat([node_explanation.unsqueeze(0), pert_node_explanation.unsqueeze(0),
                                               counter_node_explanation.unsqueeze(0)], dim=0), fairness_bound


dataset = configs.arg_parse().dataset
