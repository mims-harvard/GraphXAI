...
for layer in reversed(list(range(model.count_layers()))):
    ...
    for epoch in range(1, 201):
        model.train()
        org_out, _, _ = model.graphmask_forward(data.x, data.edge_index, batch,
                                                graphmask_mode=False)
        loss_org_pred = criterion(org_out, data.y)
        org_preds = torch.argmax(org_out, dim=-1)

        upd_out, penalty, num_masked = \
            model.graphmask_forward(data.x, data.edge_index, batch,
                                    graphmask_mode=True, return_gates=True)

        loss_upd_pred = criterion(upd_out, data.y)
        upd_preds = torch.argmax(org_out, dim=-1)

        loss = loss_fn(org_preds.float(), upd_preds.float())
