...
class Node_Explainer:
    ...
    def n_hops_A(self, n_hops):
        ...
        for i in range(n_hops - 1):
            power_adj = torch.matmul(power_adj.to('cuda'), adj.to('cuda')).cpu()  # power_adj @ adj
    ...
    def explain(self, node_idx, device, num_samples = 100, top_node = None, p_threshold=0.05, pred_threshold = 0.1):
        # print("Explaining node: " + str(node_idx))
        nA = self.n_hops_A(self.num_layers)
        _, _, _, neighbors = self.extract_n_hops_neighbors(nA, node_idx)
        neighbors = np.arange(nA.shape[0]-1)

        # neighbors = torch.from_numpy(np.arange(self.X.shape[0]))
        ...
        pred_torch = self.model.forward(X_torch.to(device), self.edge_index.to(device))[0]
        soft_pred = np.asarray([np.asarray(pred_torch.cpu()[node_].data) for node_ in range(self.X.shape[0])])

        pred_node = np.asarray(pred_torch[node_idx].cpu().data)
        ...
        for iteration in range(num_samples):
            ...
            pred_perturb_torch = self.model.forward(X_perturb_torch.to(device), self.edge_index.to(device))[0]

            soft_pred_perturb = np.asarray([np.asarray(pred_perturb_torch.cpu()[node_].data) for node_ in range(self.X.shape[0])])
            sample_bool = []
            for node in neighbors:
