...
def aggregate_explanations(
        ...
        reference_exp = None,
        feature_aggregator = torch.sum,
        node_aggregator = torch.sum,
    ...
    if ref_exp.feature_imp is not None:
        feature_imp = feature_aggregator(torch.stack([exp.feature_imp for exp in exp_list]))

    if ref_exp.node_imp is not None:
        node_imp = node_aggregator(torch.stack([exp.node_imp for exp in exp_list]))

    if ref_exp.edge_imp is not None:
        edge_imp = edge_aggregator(torch.stack([exp.edge_imp for exp in exp_list]))
